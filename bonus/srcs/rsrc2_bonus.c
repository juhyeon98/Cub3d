...
/*   Updated: 2024/04/17 16:12:45 by taeoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_down(t_map const map, size_t const y, size_t const x)
{
	if (y == 0 || y + 1 == map.h)
		...
		print_error(E_MAP_INVAL);
	return (map.map[y - 1][x] == 'D' || map.map[y + 1][x] == '0' || is_player(map.map[y + 1][x]));
}
...
int	check_right(t_map const map, size_t const y, size_t const x)
{
	if (x == 0 || map.map[y][x] == '\0')
		...
		print_error(E_MAP_INVAL);
	return (map.map[y - 1][x] == 'D' || map.map[y][x + 1] == '0' || is_player(map.map[y][x + 1]));
}
